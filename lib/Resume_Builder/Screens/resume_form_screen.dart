...
// ============================================
// FINAL RESUME SCREEN WITH DOWNLOAD
// ============================================
class FinalResumeScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          const SizedBox(height: 12),
          // Resume preview with Screenshot widget
          Expanded(
  ...
  Future<void> _downloadResume(BuildContext context) async {
    try {
      ...
      if (image != null) {
        ...
        final fileName = 'resume_${controller.selectedTemplateId.value}_$timestamp.png';
        ...
        if (Platform.isAndroid) {
          try {
            ...
            if (await downloadsDir.exists()) {
              ...
              await downloadFile.writeAsBytes(image);
                            Get.back();
                            Get.snackbar(
                '✓ Download Complete',
  ...
  Future<void> _shareResume(BuildContext context) async {
    try {
      ...
      if (image != null) {
        // Save to temp file
        final directory = await getTemporaryDirectory();
        final fileName = 'resume_${DateTime.now().millisecondsSinceEpoch}.png';
        final filePath = '${directory.path}/$fileName';
        final file = File(filePath);
        await file.writeAsBytes(image);
  ...
  // Show share dialog for downloaded file
  void _showShareDialog(BuildContext context, String filePath) {
    Get.dialog(
      AlertDialog(
        ...
        content: Text(
          'Your resume has been generated. What would you like to do?',
          style: TextStyle(color: AppColors.textSecondary),
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              Get.back();
              Share.shareXFiles([XFile(filePath)], text: 'My Resume');
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: controller.selectedTemplateAccent.value,
              foregroundColor: Colors.white,
...
// ─────────────────────────────────────────────
//  RENDERED RESUME TEMPLATES (DATA-DRIVEN)
// ─────────────────────────────────────────────

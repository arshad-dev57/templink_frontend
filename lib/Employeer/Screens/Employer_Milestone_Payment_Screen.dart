// screens/milestone_payment_screen.dart
import 'dart:convert';
  ...
  Future<void> processWalletPayment() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/api/milestones/pay-with-wallet'),
        headers: {
  ...
  Future<void> processStripePayment() async {
    ...
    try {
      ...
      final paymentResponse = await http.post(
        Uri.parse('$baseUrl/api/milestones/create-payment'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      final verifyResponse = await http.post(
        Uri.parse('$baseUrl/api/milestones/verify-payment'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          _buildSummaryRow('Platform Fee', '\$${(widget.milestone.amount * 0.05).toStringAsFixed(2)}', color: Colors.grey),
          const Padding(
          ...
          _buildSummaryRow(
            'Total', 
            '\$${(widget.milestone.amount * 1.05).toStringAsFixed(2)}',
            isBold: true,
            color: primary,
          ),
        ],
        ...
        child: Row(
          ...
          children: [
            ...
            Text(
              isWalletSelected 
                  ? 'Pay with Wallet' 
                  : 'Pay \$${(widget.milestone.amount * 1.05).toStringAsFixed(2)}',
              style: const TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.w600,
              ),
